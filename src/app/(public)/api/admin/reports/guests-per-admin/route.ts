/**
 * Guests Per Admin Report API
 *
 * GET /api/admin/reports/guests-per-admin - Generate guests per admin report
 */
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!format || format === 'json') {
      const data = await fetchGuestsPerAdmin(user.wedding_id);
      return NextResponse.json(data);
    }

    // Generate report for excel/csv
    const result = await exportGuestsPerAdmin(user.wedding_id, format as ExportFormat);
