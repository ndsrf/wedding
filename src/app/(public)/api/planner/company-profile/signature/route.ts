...
export async function POST(request: NextRequest) {
  try {
    ...
    const filename = `company-profile/${user.planner_id}/signature-${timestamp}-${randomStr}.png`;
    ...
    await prisma.weddingPlanner.update({
      ...
      data: { signature_url: uploadResult.url },
    });

    return NextResponse.json({ data: { signature_url: uploadResult.url } });
  } catch (error) {
    console.error('POST /api/planner/company-profile/signature error:', error);
...
export async function DELETE() {
  try {
    ...
    return NextResponse.json({ data: { signature_url: null } });
  } catch (error) {
    console.error('DELETE /api/planner/company-profile/signature error:', error);
