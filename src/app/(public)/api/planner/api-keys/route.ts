/**
 * Wedding Planner — API Keys
 *
 * GET  /api/planner/api-keys  — List API keys for this planner (no raw key)
 * POST /api/planner/api-keys  — Replace all existing keys and create a new one (returns raw key once)
 */
...
function handleError(error: unknown) {
  ...
  if (msg.includes('FORBIDDEN')) return NextResponse.json({ success: false, error: { code: API_ERROR_CODES.FORBIDDEN, message: 'Planner role required' } }, { status: 403 });
  console.error('[PLANNER-API-KEYS] error:', error);
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Delete all existing keys for this planner
