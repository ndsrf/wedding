/**
 * Natural Language Query Report API — Planner (cross-wedding + financials)
 * POST /api/planner/reports/query
 *
 * Accepts a natural-language question (or pre-validated SQL for exports) and
 * returns JSON data or an Excel/CSV file. Queries are scoped to the planner's
 * data via planner_id.
 */
...
import {
  executeNaturalLanguagePlannerQuery,
  executeValidatedPlannerSQL,
  ...
  try {
    ...
    if (format === 'csv') {
      ...
      return new NextResponse(Buffer.from(csv, 'utf-8'), {
        headers: {
          ...
          'Content-Disposition': `attachment; filename="planner-report-${timestamp}.csv"`,
        },
      });
    }
    ...
    return new NextResponse(new Uint8Array(buffer), {
      headers: {
        ...
        'Content-Disposition': `attachment; filename="planner-report-${timestamp}.xlsx"`,
      },
    });
  } catch (error) {
