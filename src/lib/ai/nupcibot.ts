...
7. If the message is just a greeting ("Hi", "Hello", "Hola"...) or explicitly asks what you can do / how you can help, don't just greet back: briefly introduce yourself as NupciBot and mention 2–3 example topics from the documentation above that are relevant to a ${role === 'planner' ? 'planner (e.g. managing weddings, quotes, contracts, invoices, or providers)' : 'wedding admin/couple (e.g. managing guests, seating, invitations, or the checklist)'} — vary which ones you mention, don't dump the whole list every time. Keep it to 2–3 short sentences, not a bullet list.
8. At the very end of your response, if you referenced any specific platform pages, add a block starting with exactly "[LINKS]" on its own line. Under it, list each page on its own line as: /path|Label in the response language. Example for English: /admin/guests|Guest Management. Pages you may reference include: /admin/tasting|Tasting Menu, /admin/templates|Message Templates. Only include pages you actually referenced. Omit the [LINKS] block entirely if no specific pages were mentioned.`;
}
