...
export default function AdminApiKeySection({ existingKey, mcpUrl }: Props) {
  const t = useTranslations('admin.account.apiKey');
  ...
  return (
    <section className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
      ...
      {/* Existing key info (when no new key shown) */}
      {!key && hasActiveKey && (
      ...
      <button
        onClick={generate}
        disabled={loading}
        className="px-4 py-2 rounded-lg text-sm font-semibold bg-rose-600 text-white hover:bg-rose-700 disabled:opacity-60 transition-colors"
      >
