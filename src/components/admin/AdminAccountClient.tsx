...
const CONTRACT_STATUS_LABELS: Record<string, { label: string; color: string }> = {
  ...
  SHARED: { label: 'Compartido', color: 'bg-blue-100 text-blue-700' },
  SIGNING: { label: 'Pendiente de firma', color: 'bg-amber-100 text-amber-700' },
...
const INVOICE_STATUS_LABELS: Record<string, { label: string; color: string }> = {
  ...
  PARTIAL: { label: 'Pago parcial', color: 'bg-amber-100 text-amber-700' },
...
export default function AdminAccountClient({
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Documents Section */}
      <section className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
        <h2 className="text-base font-semibold text-gray-900 mb-5">Documentos</h2>

        {!contract && !quote && invoices.length === 0 ? (
          <p className="text-sm text-gray-500">No hay documentos disponibles todavía.</p>
        ) : (
          <div className="space-y-3">
            ...
            {quote && (
              <div className="flex items-center justify-between rounded-lg border border-gray-100 bg-gray-50 px-4 py-3">
                ...
                <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${QUOTE_STATUS_LABELS[quote.status]?.color ?? 'bg-gray-100 text-gray-700'}`}>
              ...
              <div className="flex items-center justify-between rounded-lg border border-gray-100 bg-gray-50 px-4 py-3">
                <div className="flex items-center gap-3 min-w-0 flex-1">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm font-medium text-gray-900 truncate">Contrato</p>
                    <p className="text-xs text-gray-500 truncate">{contract.title}</p>
                  </div>
                </div>
                <div className="flex items-center gap-2 flex-shrink-0 ml-2">
                  <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${CONTRACT_STATUS_LABELS[contract.status]?.color ?? 'bg-gray-100 text-gray-700'}`}>
            ...
            {invoices.map((inv) => {
              ...
              return (
                <div key={inv.id} className="flex items-center justify-between rounded-lg border border-gray-100 bg-gray-50 px-4 py-3">
                  <div className="flex items-center gap-3 min-w-0 flex-1">
                    ...
                    <div className="min-w-0">
                      <p className="text-sm font-medium text-gray-900">
                        {INVOICE_TYPE_LABELS[inv.type] ?? inv.type} {inv.invoice_number}
                      </p>
                  ...
                  <div className="flex items-center gap-2 flex-shrink-0 ml-2">
                    <span className={`text-xs font-semibold px-2.5 py-1 rounded-full ${INVOICE_STATUS_LABELS[inv.status]?.color ?? 'bg-gray-100 text-gray-700'}`}>
      ...
      {pendingInvoices.length > 0 && (
        <section className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
          <h2 className="text-base font-semibold text-gray-900 mb-5">Pagos pendientes</h2>
          <div className="space-y-3">
      ...
      {/* Payment Schedule Section (from contract) */}
      {paymentSchedule.length > 0 && (
        <section className="bg-white rounded-xl border border-gray-100 shadow-sm p-6">
          <h2 className="text-base font-semibold text-gray-900 mb-1">Plan de pagos del contrato</h2>
          <p className="text-xs text-gray-500 mb-5">Calendario de pagos acordado con tu wedding planner</p>
          <div className="space-y-3">
              ...
              <div key={item.id} className="flex items-center justify-between rounded-lg border border-gray-100 bg-gray-50 px-4 py-3">
                ...
                <p className="text-sm font-semibold text-gray-900 flex-shrink-0 ml-4">
                  {item.amount_type === 'FIXED'
                    ? formatCurrency(item.amount_value)
                    : `${item.amount_value}%`}
                </p>
              </div>
            ))}
