...
interface GuestTableProps {
  ...
  onCopyWhatsAppText?: (guestId: string) => Promise<string>;
  loading?: boolean;
...
export function GuestTable({
  ...
  const [copyingWaIds, setCopyingWaIds] = useState<Set<string>>(new Set());
  const [copiedWaIds, setCopiedWaIds] = useState<Set<string>>(new Set());
  ...
  const handleCopyWhatsAppText = useCallback(async (guestId: string) => {
    if (!onCopyWhatsAppText || copyingWaIds.has(guestId)) return;
    setCopyingWaIds(prev => new Set(prev).add(guestId));
    try {
      const text = await onCopyWhatsAppText(guestId);
      await navigator.clipboard.writeText(text);
      setCopiedWaIds(prev => new Set(prev).add(guestId));
      setTimeout(() => {
        setCopiedWaIds(prev => { const s = new Set(prev); s.delete(guestId); return s; });
      }, 2000);
    } catch {
      // silently ignore clipboard errors
    } finally {
      setCopyingWaIds(prev => { const s = new Set(prev); s.delete(guestId); return s; });
    }
                ...
                {(onEdit || onDelete || onSendReminder || onSendSaveTheDate) && (
                  <td className="px-2 py-4 whitespace-nowrap text-right">
                    <div className="flex justify-end gap-0.5">
                      ...
                      {onCopyWhatsAppText && guest.channel_preference === 'WHATSAPP' && (
                        <button
                          onClick={() => handleCopyWhatsAppText(guest.id)}
                          disabled={copyingWaIds.has(guest.id)}
                          className="p-1 text-green-600 hover:text-green-900 hover:bg-green-50 rounded disabled:opacity-50"
                          title={t('admin.guests.copyWhatsAppText')}
                        >
                          {copiedWaIds.has(guest.id) ? (
                      ...
                      <div className="flex gap-1 flex-wrap">
                        ...
                        {onCopyWhatsAppText && guest.channel_preference === 'WHATSAPP' && (
                          <button
                            onClick={(e) => {
                              e.stopPropagation();
                              handleCopyWhatsAppText(guest.id);
                            }}
                            disabled={copyingWaIds.has(guest.id)}
