...
export interface NotificationsApiPaths {
  notifications: string;
  notificationRead: (id: string) => string;
  notificationsMarkRead: string;
  notificationsExport: string;
  remindersPreview: string;
  reminders: string;
}

interface NotificationsPageContentProps {
  apiPaths: NotificationsApiPaths;
  header: ReactNode;
}

export function NotificationsPageContent({ apiPaths, header }: NotificationsPageContentProps) {
  ...
  const fetchNotifications = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(`${apiPaths.notifications}?${params.toString()}`);
  ...
  }, [apiPaths.notifications, page, filters]);

  const fetchReminderPreview = useCallback(async () => {
    setReminderLoading(true);
    try {
      const response = await fetch(apiPaths.remindersPreview);
      if (!response.ok) {
        setReminderFamilies([]);
        return;
      }
  ...
  const handleMarkRead = async (notificationId: string) => {
    try {
      await fetch(apiPaths.notificationRead(notificationId), { method: 'PATCH' });
      fetchNotifications();
    } catch (error) {
  ...
  const handleBulkMarkRead = async () => {
    ...
    try {
      setLoading(true);
      await fetch(apiPaths.notificationsMarkRead, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  const handleExport = async () => {
    try {
      const response = await fetch(apiPaths.notificationsExport, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
  ...
  // Derive apiBase from reminders path for ReminderModal (strips /reminders suffix)
  const apiBase = apiPaths.reminders.replace(/\/reminders$/, '');
