...
- [ ] 9.1a. Create Guest Management Service
  - Files: src/lib/guests/crud.ts, src/lib/guests/validation.ts, src/lib/guests/audit.ts
  - Implement createFamily(input) to create family with members and generate magic token
  - Implement getFamilyWithMembers(family_id, wedding_id) to retrieve family details
  - Implement updateFamily(family_id, wedding_id, input) to update family and members
  - Implement deleteFamily(family_id, wedding_id, admin_id) to cascade delete family
  - Implement validation functions for family and member data
  - Implement audit logging for all CRUD operations
  - Purpose: Provide reusable service layer for guest family CRUD operations
  - _Leverage: Prisma client, magic link generator from task 3.2, tracking service from task 11.1_
  - _Requirements: Requirement 13 (Manual Guest Family Management), Guest Management Service (Design doc)_
  - _Prompt: Implement the task for spec wedding-mvp, first run spec-workflow-guide to get the workflow guide then implement the task: Role: Backend Developer with expertise in service layer architecture and data validation | Task: Create guest management service with CRUD functions. createFamily validates input (requires name), creates family record, generates magic token and reference code (if automated mode), creates family members, logs audit event. getFamilyWithMembers retrieves family with all members. updateFamily validates input, updates family fields, handles member updates (add/edit/delete based on input), logs audit event. deleteFamily checks for RSVP submissions, cascade deletes family/members/events/gifts, returns deletion counts and RSVP flag, logs audit event. Implement validation functions checking required fields, email format, language codes. Implement audit logging storing action, family_id, wedding_id, admin_id, timestamp, details. | Restrictions: Must validate all inputs with Zod schemas, ensure wedding_id matches for all operations, generate unique magic tokens using crypto.randomUUID(), handle reference code generation for automated payment mode, cascade delete all related data (members, events, gifts), flag families with RSVP submissions before deletion, log all CRUD actions for audit trail, handle errors gracefully with descriptive messages | Success: createFamily validates and creates families with members and tokens, getFamilyWithMembers retrieves complete family data, updateFamily handles all update scenarios correctly, deleteFamily cascades correctly and flags RSVPs, validation catches all invalid inputs, audit logs created for all operations, proper error handling with clear messages | After implementation: Mark task as [-] in tasks.md before starting, log implementation with log-implementation tool documenting guest management service functions with full signatures and validation rules, then mark as [x] when complete_

- [ ] 9.1b. Extend Guest Management API routes with CRUD endpoints
  - Files: src/app/api/admin/guests/route.ts, src/app/api/admin/guests/[id]/route.ts
  - Implement POST /api/admin/guests (create new family)
  - Implement GET /api/admin/guests/:id (get family details with members)
  - Extend PATCH /api/admin/guests/:id (update family and members)
  - Implement DELETE /api/admin/guests/:id (delete family)
  - Purpose: Provide API endpoints for manual guest family management
  - _Leverage: Guest management service from task 9.1a, auth middleware, type definitions_
  - _Requirements: Requirement 13 (Manual Guest Family Management), Wedding Admin APIs (Design doc)_
  - _Prompt: Implement the task for spec wedding-mvp, first run spec-workflow-guide to get the workflow guide then implement the task: Role: Backend API Developer with expertise in RESTful APIs and validation | Task: Extend guest API routes with CRUD operations. POST /guests validates request body (requires name, optional contact info, members array), calls createFamily service, returns created family with 201 status. GET /guests/:id validates family belongs to wedding, calls getFamilyWithMembers service, returns family data. PATCH /guests/:id validates request body, calls updateFamily service with member updates (add/edit/delete), returns updated family. DELETE /guests/:id validates family belongs to wedding, calls deleteFamily service, returns deletion result with RSVP warning flag if applicable. Protect all routes with wedding_admin role. Use consistent APIResponse format. | Restrictions: Must validate wedding_id matches session for all operations, use Zod schemas for request validation, return proper HTTP status codes (200, 201, 400, 403, 404, 409), include RSVP warning in DELETE response, handle validation errors with detailed messages, ensure multi-tenancy isolation, implement proper error handling | Success: POST creates families with validation, GET retrieves family details correctly, PATCH updates families and members correctly, DELETE cascades deletion and warns for RSVPs, all endpoints enforce wedding_id isolation, proper status codes and error messages, validation catches invalid inputs | After implementation: Mark task as [-] in tasks.md before starting, log implementation with log-implementation tool documenting extended guest CRUD APIs with request/response formats and validation rules, then mark as [x] when complete_

- [ ]* 9.1c. Write property tests for guest CRUD operations
  - Files: __tests__/properties/guest-crud.test.ts
  - **Property 1: Family creation requires name** - For any family creation attempt without a name, the system should reject with validation error
  - **Property 2: Family creation generates unique tokens** - For any created family, magic token should be unique
  - **Property 5: Family deletion cascades to related data** - For any family with members/events/gifts, deletion should remove all related data
  - **Property 6: RSVP families trigger deletion warning** - For any family with RSVP submission, deletion should flag warning
  - **Property 7: Family deletion creates audit log** - For any deleted family, audit log entry should be created
  - Purpose: Validate guest CRUD correctness properties with property-based testing
  - _Leverage: fast-check library, guest management service from task 9.1a, Jest_
  - _Requirements: Requirement 13 (Manual Guest Family Management), Correctness Properties (Design doc)_
  - _Prompt: Implement the task for spec wedding-mvp, first run spec-workflow-guide to get the workflow guide then implement the task: Role: QA Engineer with expertise in property-based testing and fast-check | Task: Write property-based tests using fast-check for guest CRUD operations. Test Property 1 by generating families with empty names and verifying rejection. Test Property 2 by creating multiple families and verifying unique magic tokens. Test Property 5 by creating families with varying numbers of members/events/gifts and verifying cascade deletion. Test Property 6 by creating families with/without RSVP events and verifying warning flag. Test Property 7 by deleting families and verifying audit log entries. Configure each test to run 100 iterations. Tag each test with "Feature: wedding-mvp-guest-crud, Property N: [property text]". | Restrictions: Must use fast-check for property generation, run minimum 100 iterations per test, test with realistic data ranges, verify all properties hold for generated inputs, tag tests with feature and property references, ensure tests are deterministic and repeatable | Success: All property tests pass with 100 iterations, properties validated across wide input ranges, tests catch edge cases, proper tagging for traceability, tests run efficiently | After implementation: Mark task as [-] in tasks.md before starting, log implementation with log-implementation tool documenting property tests with generators and assertions, then mark as [x] when complete_

- [ ]* 9.1d. Write unit tests for guest CRUD service
  - Files: __tests__/unit/lib/guests/crud.test.ts
  - Test createFamily with valid and invalid inputs
  - Test updateFamily with various update scenarios
  - Test deleteFamily with cascade deletion
  - Test validation functions
  - Test audit logging
  - Purpose: Ensure guest CRUD service functions work correctly with specific examples
  - _Leverage: Jest, Prisma test client, guest management service from task 9.1a_
  - _Requirements: Requirement 13 (Manual Guest Family Management), Testing Strategy (Design doc)_
  - _Prompt: Implement the task for spec wedding-mvp, first run spec-workflow-guide to get the workflow guide then implement the task: Role: QA Engineer with expertise in unit testing and Jest | Task: Write comprehensive unit tests for guest CRUD service. Test createFamily with valid inputs (creates family, generates token, creates members), invalid inputs (missing name, invalid email), automated vs manual payment mode. Test updateFamily with contact info updates, member additions, member updates, member deletions. Test deleteFamily with cascade deletion counts, RSVP warning flag, audit logging. Test validation functions with valid/invalid data. Test audit logging creates correct entries. Use Prisma test database with reset between tests. | Restrictions: Must test both success and failure cases, verify database state after operations, check magic token generation, verify cascade deletions, test RSVP warning logic, verify audit logs, use test database with isolation, mock external dependencies if needed | Success: All unit tests pass, comprehensive coverage of CRUD operations, validation tested thoroughly, cascade deletion verified, RSVP warnings tested, audit logging verified, tests are isolated and repeatable | After implementation: Mark task as [-] in tasks.md before starting, log implementation with log-implementation tool documenting unit tests with test cases and coverage, then mark as [x] when complete_
...
- [ ] 9.5a. Create Guest CRUD UI components
  - Files: src/components/admin/GuestFormModal.tsx, src/components/admin/GuestDeleteDialog.tsx, src/components/admin/FamilyMemberForm.tsx
  - Create GuestFormModal component for add/edit family (modal with form)
  - Create GuestDeleteDialog component for delete confirmation
  - Create FamilyMemberForm component for managing family members within form
  - Purpose: Provide UI components for manual guest family management
  - _Leverage: Guest CRUD APIs from task 9.1b, Tailwind CSS, i18n, React Hook Form_
  - _Requirements: Requirement 13 (Manual Guest Family Management), Mobile-First Design (Design doc)_
  - _Prompt: Implement the task for spec wedding-mvp, first run spec-workflow-guide to get the workflow guide then implement the task: Role: Frontend Developer specializing in React forms and modal interfaces | Task: Create GuestFormModal component with form fields for family name (required), email, phone, WhatsApp, language preference, channel preference. Include FamilyMemberForm section allowing add/edit/remove members with fields for name (required), type (adult/child/infant), age, dietary restrictions, accessibility needs. Support both "Add Guest" and "Edit Guest" modes. Create GuestDeleteDialog component showing family name, member count, and warning if family has submitted RSVP. Include cancel and confirm buttons. Use React Hook Form for validation. Style with Tailwind CSS. Integrate i18n. | Restrictions: Must be mobile-first responsive, validate required fields client-side, show clear error messages, handle loading states during API calls, display success/error notifications, support add/edit modes in same component, show RSVP warning prominently in delete dialog, use Tailwind CSS only, integrate with i18n, follow accessibility guidelines | Success: GuestFormModal displays correctly in add/edit modes, form validates required fields, member management works (add/edit/remove), form submits correctly to API, GuestDeleteDialog shows family details and RSVP warning, delete confirmation works, all components are responsive and accessible, proper error handling and user feedback | After implementation: Mark task as [-] in tasks.md before starting, log implementation with log-implementation tool documenting guest CRUD components with props and validation rules, then mark as [x] when complete_

- [ ] 9.5b. Integrate Guest CRUD UI into guests page
  - Files: src/app/admin/guests/page.tsx, src/components/admin/GuestTable.tsx
  - Add "Add Guest" button to guests page header
  - Wire up "Edit" button in GuestTable to open GuestFormModal
  - Add "Delete" button/icon to GuestTable rows
  - Integrate GuestFormModal and GuestDeleteDialog components
  - Refresh guest list after CRUD operations
  - Purpose: Enable wedding admins to manually manage guests from the UI
  - _Leverage: Guest CRUD components from task 9.5a, existing guests page from task 9.5_
  - _Requirements: Requirement 13 (Manual Guest Family Management), Mobile-First Design (Design doc)_
  - _Prompt: Implement the task for spec wedding-mvp, first run spec-workflow-guide to get the workflow guide then implement the task: Role: Frontend Developer with expertise in React state management and component integration | Task: Update guests page to add "Add Guest" button in header next to Import button. Update GuestTable component to make Edit button functional (pass onEdit prop, open GuestFormModal with family data). Add Delete button/icon to each table row, open GuestDeleteDialog on click. Integrate GuestFormModal component with add/edit modes. Integrate GuestDeleteDialog component. Handle modal open/close state. Refresh guest list after successful create/update/delete operations. Show success/error toast notifications. Maintain existing import/export/filter functionality. | Restrictions: Must maintain existing functionality (import/export/filters), add CRUD without breaking current features, handle modal state correctly, refresh data after operations, show clear user feedback, maintain mobile-first responsive design, integrate with i18n, follow accessibility guidelines | Success: "Add Guest" button opens modal in create mode, Edit button opens modal with pre-filled data, Delete button shows confirmation dialog with warnings, all CRUD operations work correctly, guest list refreshes after changes, success/error notifications display, existing features still work, fully responsive and accessible | After implementation: Mark task as [-] in tasks.md before starting, log implementation with log-implementation tool documenting integration changes and state management approach, then mark as [x] when complete_
